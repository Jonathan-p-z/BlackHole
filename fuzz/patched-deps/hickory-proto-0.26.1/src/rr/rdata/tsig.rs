    ...
    fn read_data(decoder: &mut BinDecoder<'r>, length: Restrict<u16>) -> Result<Self, DecodeError> {
        ...
        let mac_size = decoder
            ...
            .map_err(|size| DecodeError::IncorrectRDataLengthRead {
                // PATCHED (local fuzz-only vendor copy, see
                // fuzz/patched-deps/README.md): upstream used plain `-`
                // here, which panics ("attempt to subtract with
                // overflow") once `decoder.index()` has already advanced
                // past `end_idx` — the `verify_unwrap` check above only
                // proves `index + size + 6 > end_idx`, not `index <=
                // end_idx`. `saturating_sub` reports 0 instead of
                // panicking in that case; this is only ever used to
                // populate an error message's `read` field, so a
                // saturated value is harmless.
                read: end_idx.saturating_sub(decoder.index()),
                len: size as usize + 6,
            })?;
        let mac =
        ...
        let other_len = decoder
            ...
            .map_err(|size| DecodeError::IncorrectRDataLengthRead {
                // PATCHED — same underflow class as the `mac_size` check
