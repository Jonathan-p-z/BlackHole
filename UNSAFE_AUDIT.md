...
| --- | --- | --- |
| 53 | `libc::getuid()` | Yes, in principle: `rustix::process::getuid()` or `nix::unistd::getuid()` wrap this safely. **Not adopted**: `getuid(2)` takes no arguments, cannot fail, and touches no memory — there is no invariant for the caller to uphold, so the `unsafe` here is a formality rather than a real risk. Pulling in a new dependency (`rustix`/`nix`) to remove one zero-risk `unsafe` block on a single infallible syscall isn't worth the added dependency surface (an extra crate to audit and update). |
