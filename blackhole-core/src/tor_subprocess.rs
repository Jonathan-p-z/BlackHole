...
impl SubprocessTorBackend {
    ...
    pub async fn start(config: SubprocessConfig) -> Result<Self, BlackholeError> {
        ...
        let control_addr: SocketAddr = format!("127.0.0.1:{}", config.control_port)
            .parse()
            .unwrap();
        ...
        let child = Command::new(&binary_path)
            ...
            .map_err(|e| {
                BlackholeError::Tor(format!(
                    "failed to spawn tor binary at {}: {e}",
                    binary_path.display()
                ))
            })?;
...
impl TorBackend for SubprocessTorBackend {
    async fn status(&self) -> TorStatus {
        ...
        match ControlClient::connect(self.control_addr, &self.cookie_path, Duration::from_secs(5))
            .await
        {
            Ok(mut client) => match client.bootstrap_status().await {
                Ok((percent, ready_for_traffic, blocked_reason)) => TorStatus {
    ...
    async fn new_identity(&self) -> Result<(), BlackholeError> {
        if let Some(exit_status) = self.child_exit_status() {
            return Err(BlackholeError::Tor(format!(
                "cannot request a new identity: tor subprocess has exited (status: {exit_status})"
            )));
        }
        let mut client =
            ControlClient::connect(self.control_addr, &self.cookie_path, Duration::from_secs(5))
                .await?;
...
async fn tor_binary_version(binary_path: &Path) -> Result<(u32, u32, u32), BlackholeError> {
    ...
    parse_tor_version(&text).ok_or_else(|| {
        BlackholeError::Tor(format!(
...
mod tests {
    ...
    fn parses_real_tor_version_string() {
        assert_eq!(
            parse_tor_version("Tor version 0.4.8.13.\nThis is experimental software.\n"),
            Some((0, 4, 8))
        );
    }
    ...
    fn parses_version_with_git_suffix() {
        ...
        assert_eq!(
            parse_tor_version("Tor version 0.4.9.1-alpha (git-abcdef1234).\n"),
            Some((0, 4, 9))
        );
    }
